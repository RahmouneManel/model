...
def load_json(json_file):
    ...
    with open(json_file, 'r') as f:
        Landmark_to_name = json.load(f)
        return Landmark_to_name
    
    
# Function to display an image along with the top 5 classes
def display_image(image_dir, Landmark_to_name, classes):
    ...
    Landmark_names = [Landmark_to_name[i] for i in classes]

    # Plot the probabilities for the top 5 classes as a bar graph
    plt.subplot(2,1,2)

    sb.barplot(x=probs, y=Landmark_names, color=sb.color_palette()[0]);
